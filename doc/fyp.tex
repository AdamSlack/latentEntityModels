...
\[
  REWORK \quad
\]
This actually wants to be an equation calculating the proportional distributions of topics given the strength of belonging each term has to a topic and the strength of association between the word and entity.

Essentially this gives the following equation:
\[
  P(E,\underline{W},\underline{Z},\underline{\theta},\phi|\alpha,\beta) = \prod^{K}_{i=1} P(\phi_i|\beta) \prod^{M}_{j=1} P(\theta_j|\alpha)\prod^{N}_{t=1} P(Z_{j,t} | \theta_j)P(W_{j,t} | \phi_{z_{j,t}})
\]
where \(E}\) is an entity in the corpus, and \(P(W_{j_t}|E\) is the probability of entity-term association.
