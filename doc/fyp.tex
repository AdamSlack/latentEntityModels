...
\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.6]{book_list}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.6]{book_topic_distr}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.6]{entity_listing}
...
\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.6]{topic_dist_pct}
...
The corpora used in the evaluation of the GETM is the same as the corpora used to evaluate the Latent Entities. For the purpose of evaluating consistency between models, a selection of 4 corpus pairs have been constructed.

The first corpus pair consists of books that are different to each other in subject matter. The first corpus being fiction, and the second being non-fiction. The assumption is that a model trained on one corpus will be entirely different to models trained on the opposing corpus. The books for ~\ref{tab:different_books}.

For the fiction corpus of this pairing, books were only considered candidates if they were both part of the same series and by the same author. Additionally, books were required to be familiar to author of the project, this ensured that there could be some interpretation of results in context of the documents used. The non-fiction corpus was constructed by consulting an online resource for finding books discussing similar subjects. 

Six documents were used in each of the corpus pairings, ensuring that a variety of entities would be present from which models could be trained. It is unknown to what effect the corpus sizes would have, and is an aspect of potential future research. By keeping the number of documents relatively low, the processing time required to compute models is also kept to a minimum, allowing for evaluation of various configurations of documents.
...
\begin{table}[h!]
  ...
  \begin{tabular}{P{0.4\linewidth} | P{0.4\linewidth}}
    \multicolumn{2}{c}{Different}       \\
    Corpus A            & Corpus B      \\ \hline
    Philosophers Stone  & Hunger Games  \\
    Chamber of Secrets  & Catching Fire \\
    Prisoner of Azkaban & Mockingjay    \\                     
  \end{tabular}
  \caption{Two Corpora Consisting of Documents on Different Subject Matter.\label{tab:different_books}}
\end{table}
\renewcommand{\baselinestretch}{2.0}\normalsize
\renewcommand{\arraystretch}{1.0}

In novels from the same series written by the same author, it is frequent that there are re-occurring characters present throughout the documents. A corpus pair constructed from a these documents may lend themselves well for evaluating the consistency of trained models. Whilst it is expected that there will be major differences, the consistency score should be higher than completely unrelated corpora.

The same series corpus pairing was constructed by taking a series of documents, and splitting it into two alternating between corpora on selection. A series of novels was selected that are familiar to the author, for the same reasons as the novels selected in the previous corpus pairing.
...
\begin{table}[h!]
  ...
  \end{tabular}\quad\\
  \vspace{0.5cm}
  \caption{Corpora of Novels from the Same Series. \label{tab:same_series}}
\end{table}
\renewcommand{\baselinestretch}{2.0}\normalsize
\renewcommand{\arraystretch}{1.0}

Whilst the corpora of novels from the same series are expected to show some similarities, there is an expectation that a corpora consisting of different books on the same topics should produce even more similar models. Documents were selected by consulting an online resource in a similar fashion to the non-fiction corpus of the differing corpus pairing.
...
\begin{table}[h!]
  \centering
  \begin{tabular}{P{0.4\linewidth} | P{0.4\linewidth}}
    \multicolumn{2}{c}{Same Topic}\\
    Corpus A & Corpus B
    \\\hline
    Henry VIII            & Philosophers Stone \\
    Henry VIII: wives     & Philosophers Stone \\
    The Tudors: Henry VIII& Philosophers Stone
  \end{tabular}
  \caption{Corpus of Distinct Novels on the Same Subject Matter. \label{tab:getm_copora}}
\end{table}
\renewcommand{\baselinestretch}{2.0}\normalsize
\renewcommand{\arraystretch}{1.0}

A final pairing of corpus is proposed as an additional means of providing insight into the consistency of the model. Frequently narratives have summaries written for them, with key characters, descriptions, and information in the book being condensed into a reference format. This corpus could potentially serve as the greatest indicator of how useful developed models are. Should the implemented methods produce somewhat similar results on a corpus of novels and a corpus of novel summaries, then there a discussion can be held with regards to models reduction of perplexity in source materials.

This is the largest of the proposed corpus pairings, it consists of the full series of Harry Potter novels in one corpus, and a full set of Harry Potter novel summaries in the other.
