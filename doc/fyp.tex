...
The process requires a corpus to be separated into two sets, one set of positions for each mention of an entity, and another set for terms. The structure of data expected for the implemented is shown in Figure~\ref{fig:getm_data}


\renewcommand{\baselinestretch}{1.0}\normalsize
\begin{figure}
  Entity Occurrence Set
  \[
    \{e_0 : \{e_{0,0}, ..., e_{0,j}\}, ..., e_i : \{e_{i,0}, ..., e_{i,j}\}\}
  \]
  Term Occurrence Set
  \[
    \{w_0 : \{w_{0,0}, ..., w_{0,j}\}, ..., w_i : \{w_{i,0}, ..., w_{i,j}\}\}
  \]
  \caption{\label{fig:getm_data}}
\end{figure}
\renewcommand{\baselinestretch}{2.0}\normalsize
